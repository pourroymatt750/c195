...
public class CustomerDao {
    ...
    public static ObservableList<Customers> getCustomersInCountries() throws SQLException {
        ObservableList<Customers> customersList = FXCollections.observableArrayList();

        try {
            //SQL statement
            String sql = "SELECT COUNT(cu.Customer_ID) as Total_Count, co.Country " +
                    " FROM customers cu INNER JOIN first_level_divisions f ON cu.Division_ID=f.Division_ID " +
                    " INNER JOIN countries co ON f.Country_ID=co.Country_ID " +
                    " GROUP BY Country ";
            ...
            while (rs.next()) {
                //get results data
                String Country = rs.getString("Country");
                int Total_Customers = rs.getInt("Total_Count");

                Customers c = new Customers(Country, Total_Customers);
                customersList.add(c);
            }
