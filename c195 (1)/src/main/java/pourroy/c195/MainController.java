...
public class MainController {
    ...
    public void onMonthRadioBtn(ActionEvent actionEvent) throws SQLException {
        ...
        LocalTime startLocalTime = LocalTime.of(1, 0, 0);
        LocalDateTime start = LocalDateTime.of(LocalDate.now(), startLocalTime);

        LocalTime endLocaltime = LocalTime.of(23, 0, 0);
        LocalDateTime oneMonthTime = LocalDateTime.of(LocalDate.now().plusMonths(1), endLocaltime);

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        String currDateTimeStr = formatter.format(start);
        String oneMonthStr = formatter.format(oneMonthTime);


        OneMonth om = () -> {
            try {
                AppointmentDoa.getAppointmentByMonth(start);
                return;
            } catch (SQLException e) {
                e.printStackTrace();
            }
